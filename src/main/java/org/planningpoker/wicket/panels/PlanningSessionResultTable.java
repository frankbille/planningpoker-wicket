	...
	public PlanningSessionResultTable(String id, IModel<PlanningSession> model) {
		...
		IModel<List<PlanningRound>> previousRoundsModel = new LoadableDetachableModel<List<PlanningRound>>() {
			...
			protected List<PlanningRound> load() {
				...
				List<PlanningRound> previousRounds = planningSession.getPreviousRounds();

				Collections.sort(previousRounds, new Comparator<PlanningRound>() {
					public int compare(PlanningRound o1, PlanningRound o2) {
						int compare = 0;
		...
		add(new ListView<PlanningRound>("rounds", previousRoundsModel) {
			...
			protected void populateItem(ListItem<PlanningRound> item) {
				item.add(new Label<Integer>("id", new Model<Integer>(item.getIndex() + 1)));

				item.add(new PlanningRoundTitleEditableLabel("title", item.getModel()));

				item.add(new Label<String>("selectedCard", item.getModelObject().getSelectedCard()
						.getDisplayValue()));

				item.add(new Label<String>("timestamp", new DateFormatModel(
						new PropertyModel<Date>(item.getModel(), "timestamp"), "HH:mm:ss")));
			}
		});
	}
}
